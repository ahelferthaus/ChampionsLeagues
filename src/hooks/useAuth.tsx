...
interface AuthContextType {
  ...
  roles: AppRole[];
  loading: boolean;
  signUp: (email: string, password: string, fullName: string, role: SafeSignupRole) => Promise<{ error: Error | null; data?: { user: User | null } }>;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const signUp = async (email: string, password: string, fullName: string, role: SafeSignupRole) => {
    try {
      ...
      // Add the selected role after signup
      if (data.user) {
  ...
  return (
    <AuthContext.Provider value={{ user, session, profile, roles, loading, signUp, signIn, signOut }}>
      {children}
